...
**Commercial conduces (2026-09-20):** Feature 16 (`CON-001`–`CON-006`) is **documentation-confirmed**. API/HTTP **not started**. Sequenced in `docs/plan_feature_contado/IMPLEMENTATION_PLAN.md` (M1 documentation closed; M2+ implementation). Required **before** the first production release / environment gate. Key owner clarifications at M1: Administrator named-`CASH` balance exception is **conduce emission only** (not direct invoice confirm) and **excludes** default `Cliente contado`; cancellation refunds are globally **zero through net collected** (CANCEL-002 amended; runtime pending Feature 16 / plan M4).
...
- **Sales confirmation:** Billing Core plus pulled-forward optional initial payment and `dueDate` (Release 3). `Cliente contado` / `CASH` requires a full initial payment on **direct** confirmation. Paso 5 (2026-09-16, local): named `CASH` is not credit-eligible; `CREDIT` is DOP-only with limit/term snapshot; cash `dueDate` is the local confirmation day (end of day, `America/Santo_Domingo`). **Conduces (Feature 16 docs only):** Administrator named-`CASH` balance exception is conduce-emission only (CON-002); not in API yet.
- **Cancellation:** financial/non-inventory API+HTTP done with historical full-net refund behavior; **documented** global zero..net refund (CANCEL-002, 2026-09-20) awaits runtime update with Feature 16 / plan M4. Inventory restoration and Work-Order branches are mock-only until Releases 5/7.
...
Do **not** implement from the plan file when a CON-* ID exists. M1 documentation closed 2026-09-20. Remaining M2–M8 deliver domain, API, payments/CxC/cancel, PDFs, reports/history, web/mocks, and stabilization **before** the first production/environment gate.
...
- Cancellation refunds are globally zero through net collected (CANCEL-002); runtime update rides with Feature 16 / plan M4.
