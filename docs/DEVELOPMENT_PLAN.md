...
**Commercial conduces (2026-09-20 / progress 2026-09-21):** Feature 16 (`CON-001`–`CON-006`) is **documentation-confirmed**. Sequenced in `docs/plan_feature_contado/IMPLEMENTATION_PLAN.md`: **M1–M7 Completado localmente**; **M8** estabilización pendiente. Required **before** the first production release / environment gate. Key owner clarifications at M1: Administrator named-`CASH` balance exception is **conduce emission only** (not direct invoice confirm) and **excludes** default `Cliente contado`; cancellation refunds are globally **zero through net collected** (CANCEL-002; API M4 + cancel UI M7).
...
- **Sales confirmation:** Billing Core plus pulled-forward optional initial payment and `dueDate` (Release 3). `Cliente contado` / `CASH` requires a full initial payment on **direct** confirmation. Paso 5 (2026-09-16, local): named `CASH` is not credit-eligible; `CREDIT` is DOP-only with limit/term snapshot; cash `dueDate` is the local confirmation day (end of day, `America/Santo_Domingo`). **Conduces (Feature 16):** Administrator named-`CASH` balance exception is conduce-emission only (CON-002); API M3–M6 + web/mocks M7 Completado localmente.
...
Do **not** implement from the plan file when a CON-* ID exists. **M1–M7 Completado localmente (2026-09-20 / 2026-09-21).** Remaining **M8** estabilización + evidencia de walkthrough HTTP vivo **before** the first production/environment gate.
