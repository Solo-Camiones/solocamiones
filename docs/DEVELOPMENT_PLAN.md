...
| Release 3 remaining                                                                                                               | Feature 12 account-statement PDF (`STMT-001`) remains required. Aging/collections stay deferred as specified. Inventory/WO cancellation is R5/R7, not this remainder.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | Generate the account statement from `/receivables`                                                                                                                                                                     | —                                                                       |
| **Pre-production business change set**                                                                                            | **Specified 2026-09-15. Pasos 2–7 done locally:** domain migration; customer write APIs/UI/mock (`CUST-004`–`007`); tax-exclusive ITBIS (`SALE-009`/`010`) and billing cost removal (`COST-006`); confirmation engine (`SALE-005` / `CUST-005`, cash vs credit, limit, term snapshot, Seller 403 on later payments/CxC); convertible quotes (`QUOTE-001`/`002`); and derived `ABONADO`, issued date, and approved customer/invoice AR filters (`PAY-006`/`PAY-007`). Remaining steps 8–10: statement (`STMT-001`), PDF profile (`DOC-001`), and stabilization. Complete this set **before** separate environment configuration. Source of truth is the feature files, sequenced in `docs/pre_production_business_changes/IMPLEMENTATION_PLAN.md`. | Paso 7 shipped locally: CxC defaults to open balances and exposes only the approved customer and invoice filters. Seller invoice projections remain financially restricted. Statement/PDF v4 not started | Prototype POS tax-exclusive and quotes; Seller CxC hidden to match HTTP |
...
- **CxC:** ledger, derived `ABONADO` states, issued date, and server-side customer/invoice filters are done. The invoice list and customer summary remain limited to open balances; payment-state, issued-date, and currency filters are intentionally unavailable by owner decision. Paso 5 made later `POST /payments` and `GET /receivables` Administrator-only (Seller 403; nav/deep links denied). Seller invoice list/detail omit payment state, paid, refunds, and balance. Pre-production still open: account-statement PDF (`STMT-001`) before environment configuration.
...
Implementation order: documentation (done) → migration (Paso 2 done locally) → customer authorization (Paso 3 done locally) → ITBIS/cost capture (Paso 4 done locally) → credit confirmation (Paso 5 done locally) → quotes (Paso 6 done locally) → CxC/states (Paso 7 done locally) → statement PDF → invoice/quote PDFs → pre-environment gate.
