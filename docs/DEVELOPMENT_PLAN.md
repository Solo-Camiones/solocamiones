...
**Commercial conduces (2026-09-20 / closed locally 2026-09-21):** Feature 16 (`CON-001`–`CON-006`) is **documentation-confirmed**. Sequenced in `docs/plan_feature_contado/IMPLEMENTATION_PLAN.md`: **M1–M8 Completado localmente**. Required **before** the first production release / environment gate; owner `Verificado` / formal release authorization still pending. Key owner clarifications at M1: Administrator named-`CASH` balance exception is **conduce emission only** (not direct invoice confirm) and **excludes** default `Cliente contado`; cancellation refunds are globally **zero through net collected** (CANCEL-002; API M4 + cancel UI M7).

**AI Assistant (2026-09-22):** Feature 17 (`AI-001`–`AI-010`) is **documentation-confirmed** in `FEATURES/17_AI_ASSISTANT.md`. Technical sequence: `docs/chatbot_implementation/IMPLEMENTATION_PLAN.md` (**M0–M8 done locally through 2026-09-24** — OpenAI adapters, persistence, corpus/sync, tools, orchestrator, HTTP/SSE, web panel, **security/ops** in `docs/assistant-ops/`; M9 not started; **provider privacy confirmed by owner 2026-09-24** with documented residuals; guides pending owner approval). This is a **post-stabilization pilot** for `ADMINISTRATOR` only. It **does not** amend Paso 10, Feature 16, or the first-production / environment gate.
...
| **Pre-production business change set**                                                                                            | **Specified 2026-09-15. Pasos 2–9 done locally:** domain migration; customer write APIs/UI/mock (`CUST-004`–`007`); tax-exclusive ITBIS (`SALE-009`/`010`) and billing cost removal (`COST-006`); confirmation engine (`SALE-005` / `CUST-005`, cash vs credit, limit, term snapshot, Seller 403 on later payments/CxC); convertible quotes (`QUOTE-001`/`002`); derived `ABONADO`, issued date, and approved customer/invoice AR filters (`PAY-006`/`PAY-007`); account statement PDF (`STMT-001`); and PDF profile (`DOC-001`: code-only corporate/payment profile, `internal-v4`, quote PDF, invoice without payment state/balance/balance timestamp). Paso 9 quedó verificado y sus muestras fueron aprobadas el 2026-09-17. Remaining step 10: stabilization. **Additionally required before first production release (2026-09-20):** commercial conduces Feature 16 — **M1–M8 Completado localmente** (API + web/mocks + estabilización; walkthrough owner OK 2026-09-21; `Verificado` pendiente) (`docs/plan_feature_contado/IMPLEMENTATION_PLAN.md`). Complete this set **before** separate environment configuration. Source of truth is the feature files; conduce IDs live in `FEATURES/16_CONDUCES.md`. | Paso 9 shipped locally: Seller/Administrator download invoice and issued-quote PDFs; Mechanic denied. Account statement remains Administrator-only. **M7 (2026-09-21):** POS dual factura/conduce, detalle PDF/facturar/pago/cancel, tab Conduce, mocks CON-* / CANCEL-002. **M8:** CxC UI filtro `FAC-`/`CON-`. | Prototype POS tax-exclusive and quotes; Seller CxC hidden to match HTTP; seed conduce `CON-000001` |
...
| **Feature 17 AI Assistant (post-stabilization pilot)**                                                                            | **M0 docs + M1–M8 done locally (2026-09-24):** OpenAI foundations, persistence, corpus/sync, tools, orchestrator, HTTP/SSE API, web panel, **security/ops** (`docs/assistant-ops/`, global quota, Prometheus `/metrics`). Provider privacy **owner-confirmed** 2026-09-24. Guides pending owner approval. M9 not started. Does **not** block first production release.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | Capability `assistant` on in HTTP; launcher/panel for Administrator when API flag allows                                                                                                  | No chatbot mock; capability off                                                 |
...
- **Sales confirmation:** Billing Core plus pulled-forward optional initial payment and `dueDate` (Release 3). `Cliente contado` / `CASH` requires a full initial payment on **direct** confirmation. Paso 5 (2026-09-16, local): named `CASH` is not credit-eligible; `CREDIT` is DOP-only with limit/term snapshot; cash `dueDate` is the local confirmation day (end of day, `America/Santo_Domingo`). **Conduces (Feature 16):** Administrator named-`CASH` balance exception is conduce-emission only (CON-002); M1–M8 Completado localmente.
...
Owner confirmed commercial conduces on the same sales aggregate. Canonical IDs: `CON-001`–`CON-006` in `FEATURES/16_CONDUCES.md`. Technical sequence: `docs/plan_feature_contado/IMPLEMENTATION_PLAN.md`.

Do **not** implement from the plan file when a CON-* ID exists. **M1–M8 Completado localmente (2026-09-20 / 2026-09-21).** Owner browser walkthrough OK. Formal `Verificado` / first-production authorization still pending. Paso 10 of the pre-production change set remains open separately.
