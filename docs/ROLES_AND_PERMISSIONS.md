...
| Quotes | Create, edit (`QUOTE_DRAFT`), issue, duplicate, or convert a quote | ALLOW | ALLOW | DENY | QUOTE-001, QUOTE-002. Conversion uses confirmation authorization. Expired quotes cannot convert. |
...
| Sales | Enter negotiated final prices and confirm eligible cash sales | ALLOW | ALLOW | DENY | COST-002, SALE-001..008, SALE-010, LINE-001..006. Cash confirmation must settle the gross total. |
...
| Payments | Record later, additional, partial, multiple, or mixed-method collections on a completed invoice | ALLOW | DENY | DENY | PAY-001..007. Seller cash confirmation payment is the Sales cash-confirm row above, not this collection permission. |
| Payments | View payment movement history (amounts, dates, methods, references, actors) | ALLOW | DENY | DENY | PAY-005, PAY-007. Seller may still see derived payment state and outstanding balance on the invoice. |
| Accounts Receivable | Open AR list, filters, and receivables endpoints | ALLOW | DENY | DENY | PAY-007. Deep links and direct HTTP are denied for Seller. |
...
The Seller performs normal inventory, customer (`CASH` create/edit), Draft, quote, reservation, negotiated-price, cash-sale, and eligible credit-sale work using maintained catalogs, and may establish the initial observed composition of a newly received assembly under HIER-011. The Seller may confirm a DOP credit sale to a customer already classified `CREDIT` by an Administrator, with zero initial payment. The Seller cannot create or edit credit terms, open Accounts Receivable, record later collections, view payment movements, or see acquisition cost or profitability on billing. The Seller cannot maintain catalogs. Confirming the eligible sale of an installed item may create a new Pending Dismantling Work Order or reuse an appropriate active Pending or In Progress order; the Seller cannot correct an initial baseline or Completed invoice currency, create or complete a standalone physical Work Order, or directly change hierarchy after the receipt baseline.
