...
**Commercial conduces (2026-09-20, documentation):** Feature 16 adds planned document events `CONDUCE_ISSUED`, `QUOTE_CONVERTED_TO_CONDUCE`, and `CONDUCE_INVOICED` on the same operation timeline (CON-006). Not implemented until the owning milestones.
...
- [ ] Add history projections per item/invoice/order as needed. _(Invoice detail GET + HTTP UI timeline done for document-level events; draft/line churn excluded; payment events Administrator-only; item/Work Order projections remain open)_
