...
**Immutable facts:** customer snapshot (including cash/credit type), lines, quantities, prices, stored base/ITBIS/gross, currency, `FAC-` and origin `COT-` / planned origin `CON-`, business dates (`confirmedAt` / issue / due / planned `invoiceIssuedAt`), confirmation seller (invoices and conduces) or quote issuer (issued quotes), and cancellation mark/reason/date.
