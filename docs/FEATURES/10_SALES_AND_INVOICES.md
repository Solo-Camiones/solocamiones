...
**Administrator seller-sales report (2026-09-18, local pull-forward; Conduces integrated 2026-09-21):** Finanzas y control exposes `/seller-sales` for Administrator only. `GET /api/sales/reports/seller-sales` paginates like other sales lists (`page` / `pageSize`, default 10/max 100) and `.pdf` returns the full filtered range. Both include `COMPLETED` invoices, emitted `CONDUCE` documents and outstanding `QUOTE_ISSUED` quotes in `America/Santo_Domingo` date bounds, with optional `sellerUserId`. Cancelled documents and drafts are excluded; conversion from `COT-` to `CON-`/`FAC-` and from `CON-` to `FAC-` preserves one aggregate and does not double-count it (CON-006). PDF is data-only (no commission formula). This is outside the generic reporting items in `FUTURE_ROADMAP.md`.
...
**Status:** CONFIRMED  
**Actors:** Seller, Administrator  
**Requirement:** A quote is a stage of the same sales aggregate that later becomes an invoice or a conduce and then optionally an invoice. Supported transitions include `QUOTE_DRAFT → QUOTE_ISSUED → COMPLETED` and `QUOTE_ISSUED → CONDUCE → COMPLETED`. Conversion is a state transition, not a copy into a new draft. The resulting operation keeps the same internal id, customer, currency, `applyItbis`, lines, quantities, prices, and notes. Direct convert-to-invoice assigns `FAC-` and preserves origin `COT-`. Convert-to-conduce assigns `CON-` without `FAC-` until a later CON-003 conversion; fiscal for the invoice is chosen at that later step.
...
**Main Flow:** User creates a quote draft, edits it, issues it (assigns `COT-`), then converts through confirmation to invoice or to conduce.
