...
**Period KPIs (ventas / cobros / CxC):** The Administrator Rentabilidad screen also shows period KPIs for sales cash, sales credit, total sales, and net collected by payment method (`CASH` / `TRANSFER` / `CHECK`), all in `DOP`, plus the current open accounts-receivable total (DOP primary; USD separately when present). Contado vs crédito uses confirmation `saleCondition` (full settlement → cash), not customer type. Cancelled sales are excluded from sales and collected. USD amounts use the sale’s stored FX rate; without a rate they are omitted (same rule as collected today). Collection by method buckets on payment `effectiveDate`. Series remain client-composed from `GET /api/sales` (no dedicated aggregation endpoint); open AR totals come from `GET /api/sales/receivables` customer summary. **Conduces (Feature 16 / CON-006, M6 2026-09-21):** period KPIs and profitability recognize an emitted conduce once at `confirmedAt` / conduce emission (including USD FX at that moment) and do not double-count when the same operation later receives `FAC-`. UI labels use “Ventas” (contado/crédito/total) rather than Facturado-only.
...
Administrator Rentabilidad also shows period KPIs for **sales** cash vs credit (commercial `saleCondition` at confirmation, not customer type) and **collected** by payment method (`CASH` / `TRANSFER` / `CHECK` by payment `effectiveDate`), all in DOP, plus the current open **accounts receivable** total (DOP primary, USD shown separately when present; never converted). Cancelled sales are excluded from sales and collected totals. USD amounts without a stored rate are omitted from DOP figures, same as cobrado neto. Emitted conduces count once from recognition; conversion to `FAC-` does not duplicate (CON-006).
