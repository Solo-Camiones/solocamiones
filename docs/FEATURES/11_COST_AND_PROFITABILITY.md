...
**Period KPIs (facturado / cobros / CxC):** The Administrator Rentabilidad screen also shows period KPIs for invoiced cash, invoiced credit, total invoiced, and net collected by payment method (`CASH` / `TRANSFER` / `CHECK`), all in `DOP`, plus the current open accounts-receivable total (DOP primary; USD separately when present). Contado vs crédito uses confirmation `saleCondition` (full settlement → cash), not customer type. Cancelled invoices are excluded from invoiced and collected. USD amounts use the invoice’s stored FX rate; without a rate they are omitted (same rule as collected today). Collection by method buckets on payment `effectiveDate`. Series remain client-composed from `GET /api/sales` (no dedicated aggregation endpoint); open AR totals come from `GET /api/sales/receivables` customer summary. **Conduces (Feature 16, not started):** when implemented, period KPIs and profitability must recognize an emitted conduce once at `confirmedAt` / conduce emission (including USD FX at that moment) and must not double-count when the same operation later receives `FAC-` (CON-006). Labels may widen from “Facturado” to “Ventas” where needed for correctness.
...
Administrator Rentabilidad also shows period KPIs for **invoiced** cash vs credit (commercial `saleCondition` at confirmation, not customer type) and **collected** by payment method (`CASH` / `TRANSFER` / `CHECK` by payment `effectiveDate`), all in DOP, plus the current open **accounts receivable** total (DOP primary, USD shown separately when present; never converted). Cancelled invoices are excluded from invoiced and collected totals. USD amounts without a stored rate are omitted from DOP figures, same as cobrado neto.
