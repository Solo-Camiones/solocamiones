...
import {
  QUOTE_PDF_INTERNAL_NOTICE,
  QUOTE_PDF_THANK_YOU,
  QUOTE_PDF_TITLE,
} from './constants.js';
import type { QuotePdfFacts, QuotePdfRenderer } from './types.js';
...
function money(amount: string, currency: QuotePdfFacts['currency']): string {
...
const HEADER_CONTACT_COLUMN_GAP = 12;
const HEADER_CONTACT_COLUMN_WIDTH = (HEADER_CONTACT_WIDTH - HEADER_CONTACT_COLUMN_GAP) / 2;
const HEADER_CONTACT_ROW_HEIGHT = 15;
...
function headerContactColumnX(column: 0 | 1): number {
  return HEADER_CONTACT_X + column * (HEADER_CONTACT_COLUMN_WIDTH + HEADER_CONTACT_COLUMN_GAP);
}

function headerContactRowY(row: number): number {
  return HEADER_CONTACT_GRID_Y + row * HEADER_CONTACT_ROW_HEIGHT;
}

function drawQuoteHeader(facts: QuotePdfFacts, document: PdfDocument): number {
  ...
  const leftColumnX = headerContactColumnX(0);
  const rightColumnX = headerContactColumnX(1);
  contactRow(
    document,
...
function drawQuoteFooter(document: PdfDocument, page: number, pageCount: number): void {
  const left = document.page.margins.left;
  const right = document.page.width - document.page.margins.right;
  ...
  document
    ...
    .font('Helvetica')
    .fontSize(6.2)
    .text(profileLine, left, footerTop + 6, {
      width: right - left,
    });
  document
    .fillColor(BRAND_NAVY)
  ...
  document
    ...
    .font('Helvetica')
    .fontSize(6.5)
    .text(QUOTE_PDF_INTERNAL_NOTICE, left + 300, footerTop + 46, {
      width: right - left - 300,
    });
...
function writeQuoteDocument(facts: QuotePdfFacts, document: PdfDocument): void {
  const left = document.page.margins.left;
  const right = document.page.width - document.page.margins.right;
  let y = drawQuoteHeader(facts, document);
  ...
  document
    ...
    .stroke();
  const metadata = [
    ['Emitida', formatDateTime(facts.quoteIssuedAt)],
    ['Vigente hasta', formatDate(facts.quoteExpiresAt)],
  ...
  facts.lines.forEach((line, index) => {
    ...
    if (y + rowHeight > contentBottom(document) - 8) {
      document.addPage();
      document
        .fillColor(BRAND_NAVY)
        .font('Helvetica-Bold')
        .fontSize(9)
        .text(`${facts.quoteNumber} - continuación`, left, 42);
  ...
  if (y + TOTALS_AND_SIGNATURES_HEIGHT > contentBottom(document)) {
    document.addPage();
    y = 55;
  } else {
    y += 14;
  ...
  document.roundedRect(totalsX, y, 225, 82, 6).fillAndStroke(LIGHT_BLUE, BORDER);
  const totals = [
  ...
  document
    ...
    .font('Helvetica-Oblique')
    .fontSize(8)
    .text(QUOTE_PDF_THANK_YOU, left, y, {
      width: right - left,
      align: 'center',
    });
  y += 35;
  ...
  for (let page = range.start; page < range.start + range.count; page += 1) {
    ...
    drawQuoteFooter(document, page + 1, range.count);
  }
}
  ...
  render(facts) {
    if (facts.status !== 'QUOTE_ISSUED') {
      return Promise.reject(
        new Error(`Unsupported quote PDF status: ${String(facts.status)}`),
      );
    }
    return new Promise((resolve, reject) => {
      ...
      document.info.Title = facts.quoteNumber;
      document.info.Subject = QUOTE_PDF_TITLE;
      const chunks: Buffer[] = [];
      ...
      writeQuoteDocument(facts, document);
      document.end();
    });
  },
};
