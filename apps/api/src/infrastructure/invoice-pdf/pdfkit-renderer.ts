...
function drawTikTokIcon(document: PdfDocument, x: number, y: number): void {
  drawContactGlyph(document, CONTACT_ICON_PATHS.tiktok, x, y, 'nonzero');
}
...
const HEADER_CONTACT_COLUMN_GAP = 10;
// WhatsApp and email need more width than Facebook/TikTok handles.
const HEADER_CONTACT_LEFT_WIDTH = 148;
const HEADER_CONTACT_RIGHT_WIDTH =
  HEADER_CONTACT_WIDTH - HEADER_CONTACT_LEFT_WIDTH - HEADER_CONTACT_COLUMN_GAP;
const HEADER_CONTACT_ROW_HEIGHT = 18;
...
function headerContactRightX(): number {
  return HEADER_CONTACT_X + HEADER_CONTACT_LEFT_WIDTH + HEADER_CONTACT_COLUMN_GAP;
}
...
function drawInvoiceHeader(facts: InvoicePdfFacts, document: PdfDocument): number {
  ...
  const leftColumnX = HEADER_CONTACT_X;
  const rightColumnX = headerContactRightX();
  contactRow(
    document,
    drawWhatsAppIcon,
  ...
  if (facts.originQuoteNumber) {
    document.text(facts.originQuoteNumber, 405, 102, {
      width: right - 405,
      align: 'right',
    });
  }
...
function drawInvoiceFooter(document: PdfDocument, page: number, pageCount: number): void {
  const left = document.page.margins.left;
  const right = document.page.width - document.page.margins.right;
  ...
  document
    ...
    .font('Helvetica')
    .fontSize(6.5)
    .text(INVOICE_PDF_INTERNAL_NOTICE, left + 300, footerTop + 46, {
      width: right - left - 300,
    });
...
function writeInternalV4Document(facts: InvoicePdfFacts, document: PdfDocument): void {
  const left = document.page.margins.left;
  const right = document.page.width - document.page.margins.right;
  let y = drawInvoiceHeader(facts, document);
  ...
  for (let page = range.start; page < range.start + range.count; page += 1) {
    ...
    drawInvoiceFooter(document, page + 1, range.count);
  }
}

const TEMPLATE_WRITERS: Readonly<Record<string, TemplateWriter>> = {
  [INVOICE_PDF_TEMPLATE_V4]: writeInternalV4Document,
};
